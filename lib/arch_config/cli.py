...
def cmd_validate(args) -> int:
    ...
    for e in errors:
        print(f"[ERR ] {e}")
    if not errors:
        print("[OK] config schema looks good")
    return code
...
def cmd_diff(args) -> int:
    ...
    if args.strict:
        ...
        print("These packages would be removed by strict pruning.")
        print_prune_plan(
            profile,
...
def cmd_generated(args) -> int:
    ...
    if args.generated_cmd == "list":
        ...
        for m in state.mounts:
            ...
            if m.automount:
                print(
                    f"system:{m.automount_unit} -> /etc/systemd/system/{m.automount_unit}"
                )
        return 0
    if args.generated_cmd == "show":
        ...
        for m in state.mounts:
            if name in {m.mount_unit, Path(m.mount_unit).name}:
                print(render_mount_unit(m), end="")
                return 0
            if name in {m.automount_unit, Path(m.automount_unit).name}:
                print(render_automount_unit(m), end="")
                return 0
...
def cmd_prune(args) -> int:
    ...
    print_prune_plan(
        _profile(args), include_aur=not args.no_aur, include_orphans=args.remove_orphans
    )
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    p = sub.add_parser("check-generated")
    ...
    p.add_argument("--aur", action="store_true")
    p.add_argument("--helper", default="yay")
    p.add_argument("--dry-run", action="store_true")
    p.add_argument("-y", "--yes", action="store_true")
    p.add_argument("--strict", action="store_true")
    p.add_argument("--no-prune-aur", action="store_true")
    p.add_argument("--remove-orphans", action="store_true")
    p.add_argument("--no-prune-files", action="store_true")
    ...
    p.add_argument("--dry-run", action="store_true")
    p.add_argument("-y", "--yes", action="store_true")
    p.add_argument("--no-aur", action="store_true")
    p.add_argument("--remove-orphans", action="store_true")
    ...
    gl.set_defaults(func=cmd_generated)
    gs = gsub.add_parser("show")
    gs.add_argument("name")
    gs.set_defaults(func=cmd_generated)
