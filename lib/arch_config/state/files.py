...
def files_state_path(profile: str, *, pending: bool = False) -> Path:
    return state_json_path(profile, "files.json", pending=pending)
...
def stale_managed_files(
    ...
    for target in sorted(set(old_files) - set(current_files)):
        raw = old_files[target]
...
def print_stale_managed_files(entries: list[dict[str, Any]]) -> None:
    if not entries:
        return

    print("Stale managed files cleanup preview:")
    ...
    for entry in entries:
        ...
        if unit_scope and unit:
            print(f"    unit cleanup: {unit_scope}:{unit}")

        print("    action: remove managed file")
